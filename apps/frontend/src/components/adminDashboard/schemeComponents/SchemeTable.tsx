    ...
    <Card className="rounded-2xl border border-border dark:border-border/50 bg-card/70 dark:bg-card/60 backdrop-blur-xl shadow-lg overflow-hidden">
      ...
      <div className="p-5 border-b border-border dark:border-border/50 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <select
            className="bg-card/50 dark:bg-card/50 border border-border/50 dark:border-border/10 rounded-xl px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-ring/20 transition-all cursor-pointer shadow-sm min-w-35"
      ...
      {/* Table */}
      <div className="max-h-125 overflow-y-auto alert-custom-scrollbar">
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-4 items-center">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground max-w-sm truncate mt-0.5">
                          {scheme.description}
                        </div>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4 text-xs">
                    <Badge
                      className={`px-2.5 py-0.5 rounded-lg border shadow-none font-bold text-[10px] uppercase tracking-wider ${
                        isPublished
                  ...
                  {/* Created On */}
                  <td className="px-6 py-4 text-muted-foreground dark:text-muted-foreground font-medium whitespace-nowrap">
                      ...
                      <>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              onClick={() =>
                                handleStatusAction(schemeId, "publish")
                              }
                              disabled={loadingId === schemeId}
                              className="h-8 px-3 text-[10px] font-bold bg-primary hover:bg-primary text-primary-foreground rounded-lg shadow-sm"
                            >
                          ...
                          <TooltipTrigger asChild>
                            {confirmDeleteId === schemeId ? (
                              <div className="flex items-center gap-1 animate-in slide-in-from-right-2 duration-300">
                                <Button
                                  size="sm"
                                  className="text-[10px] h-8 bg-destructive hover:bg-destructive text-primary-foreground rounded-lg"
                    ...
                    {isPublished && (
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            onClick={() =>
                              handleStatusAction(schemeId, "archive")
                            }
                            disabled={loadingId === schemeId}
                            className="h-8 px-3 text-[10px] font-bold bg-destructive hover:bg-destructive text-primary-foreground rounded-lg shadow-sm"
                          >
                    ...
                    {isArchived && (
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            onClick={() =>
                              handleStatusAction(schemeId, "restore")
                            }
                            disabled={loadingId === schemeId}
                            className="h-8 px-3 text-[10px] font-bold bg-primary hover:bg-primary text-primary-foreground rounded-lg shadow-sm"
                          >
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t border-border dark:border-border/50 text-sm">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={handlePrev}
            disabled={page === 1}
            className={`rounded-xl border-border dark:border-border/10 ${page === 1 ? "cursor-not-allowed opacity-50" : "hover:border-primary hover:text-primary"}`}
          >
            Prev
          </Button>
          <Button
            ...
            className={`rounded-xl border-border dark:border-border/10 ${page === totalPages || total === 0 ? "cursor-not-allowed opacity-50" : "hover:border-primary hover:text-primary"}`}
          >
            Next
          </Button>
        </div>
