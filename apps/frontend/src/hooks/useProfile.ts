...
import {
  ProfileState,
  initialState,
} from "../app/citizenDashboard/profile/type";
import { profileApi } from "../components/api/profileApi";
import type { ProfileDocument } from "../components/api/profileApi";
...
const FIELD_TO_SECTION: Record<string, string> = {
  ...
  accountHolderName: "passbook",
  accountNumber: "passbook",
  bankName: "passbook",
  branchName: "passbook",
  ifscCode: "passbook",
...
export function useProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const documentsJson = await profileApi.getDocuments();
        if (documentsJson.success && documentsJson.data) {
        ...
        if (json.success && json.data) {
          ...
          const savedState = {
            ...
            passbook: !!data.accountNumber || !!data.bankName,
          };
          ...
          setSectionCompletion({
            ...
            passbook: savedState.passbook ? 100 : 0,
          });
        }
      } catch (error) {
  ...
  const saveSection = async (sectionId: string) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        toast.success(
          `${sectionId.charAt(0).toUpperCase() + sectionId.slice(1).replace(/([A-Z])/g, " $1")} section saved successfully!`,
          {
  ...
  const completionStats = useMemo(() => {
    ...
    const activeSections = isFarmer
      ? [...coreSections, "agriculture"]
      : coreSections;

    const completedCount = activeSections.filter(
      (s) => sectionCompletion[s] === 100,
    ).length;
    ...
    return {
      ...
      totalFields: activeSections.length,
    };
