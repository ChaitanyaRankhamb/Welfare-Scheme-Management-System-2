...
exports.recommendSchemesService = exports.RecommendSchemesService = void 0;
class RecommendSchemesService {
    /**
     * Generates a list of recommended schemes based on user profile
     */
    getRecommendations(profile, allSchemes) {
        return allSchemes
            .map(scheme => {
    ...
    calculateMatch(profile, scheme) {
        ...
        if (!allowedStatuses.includes(profile.employmentStatus)) {
            return { score: 0, reasons: [] };
        }
        // 2. Age Filter (Hard)
        if (criteria.age) {
            const userAge = this.calculateAge(profile.dateOfBirth);
            if (criteria.age.min && userAge < criteria.age.min)
                return { score: 0, reasons: [] };
            if (criteria.age.max && userAge > criteria.age.max)
                return { score: 0, reasons: [] };
        }
...
exports.RecommendSchemesService = RecommendSchemesService;
exports.recommendSchemesService = new RecommendSchemesService();
