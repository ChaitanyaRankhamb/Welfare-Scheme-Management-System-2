...
import { EmploymentStatus } from '../entity/schemes/scheme.entity';
...
async function migrate() {
  try {
    ...
    for (const scheme of schemes) {
      ...
      (scheme as any).eligibilityCriteria = criteria;
...
function inferEmploymentFromScheme(scheme: any): EmploymentStatus[] {
  ...
  const results: EmploymentStatus[] = [];
  ...
  return uniqueResults.length > 0 ? (uniqueResults as EmploymentStatus[]) : ['other'];
}
