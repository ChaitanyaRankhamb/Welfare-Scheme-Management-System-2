...
export async function compareSchemes(schemeNames: string[]) {
  ...
  try {
    ...
    const fetchPromises = limitedNames.map(async (name) => {
      ...
      let scheme = await schemeRepository.findSchemeByName(normalized);
      if (!scheme) {
        scheme = await schemeRepository.findFuzzySchemeByName(normalized);
      }
    ...
    throw new AppError("An error occurred while generating the scheme comparison.", 500);
  }
}
