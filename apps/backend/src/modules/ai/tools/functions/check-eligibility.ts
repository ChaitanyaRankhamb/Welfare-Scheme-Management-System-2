...
import { SchemeEligibilityCriteria } from "../../../../entity/schemes/scheme.entity";
import { UserId } from "../../../../entity/user/userId";
import { AppError } from "../../../../Error/appError";
import { getUserProfileData, ProfileServiceResponseType } from "../../../citizenDashboardData/profile.service";
...
async function compareEligibility(
  eligibilityData: SchemeEligibilityCriteria,
  profileData: ProfileServiceResponseType
) {
  const eligible: EligibilityItem[] = [];
  const nonEligible: EligibilityItem[] = [];
  ...
  const addResult = (
    ...
    if (isMet) eligible.push(item);
    else nonEligible.push(item);
  };
  ...
  addResult(
    "Age",
    ageMet,
    `${eligibilityData.age.min} - ${eligibilityData.age.max} years`,
    `${profileData.age} years`,
    `Age ${profileData.age} is within the required range.`,
    `Required age range is ${eligibilityData.age.min}-${eligibilityData.age.max}, but you are ${profileData.age}.`
  );
  ...
  addResult(
    "Annual Income",
    incomeMet,
    `₹${eligibilityData.income.min} - ₹${eligibilityData.income.max}`,
    `₹${profileData.annualIncome}`,
    `Income ₹${profileData.annualIncome} meets the eligibility threshold.`,
    `Income limit is ₹${eligibilityData.income.max}, but your income is ₹${profileData.annualIncome}.`
  );
  ...
  addResult(
    ...
    `This scheme is specifically for ${eligibilityData.gender}, but your profile says ${profileData.gender}.`
  );

  // 4. Employment
  const pStatus = profileData.employmentStatus.toLowerCase();
  const employmentMet = eligibilityData.employment.employmentStatus.some(
    (s) => s.toLowerCase() === pStatus
  );
  addResult(
    "Employment Status",
    employmentMet,
    eligibilityData.employment.employmentStatus.join(", "),
    profileData.employmentStatus,
    `Employment condition met.`,
    `Scheme is for ${eligibilityData.employment.employmentStatus.join(", ")}, but your status is ${profileData.employmentStatus}.`
  );
  ...
  if (eligibilityData.location.states.length > 0) {
    ...
    addResult(
      "State Residency",
      stateMet,
      eligibilityData.location.states.join(", "),
      profileData.location.state || "Not Provided",
      `State residency requirement met.`,
      `Residency in ${eligibilityData.location.states.join(", ")} required.`
    );
  }
  ...
  if (eligibilityData.social.caste.length > 0) {
    ...
    addResult(
      "Caste Category",
      casteMet,
      eligibilityData.social.caste.join(", "),
      profileData.casteCategory,
      `Category requirement met.`,
      `Scheme is for ${eligibilityData.social.caste.join(", ")}, but you fall under ${profileData.casteCategory}.`
    );
  }
...
export async function checkEligibilityForScheme(userIdStr: string, args: { schemeName: string }) {
  const schemeName = args.schemeName;
