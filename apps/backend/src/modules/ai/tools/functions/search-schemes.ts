...
import { normalizeQuery } from "../../service/query-normalization.service";
import { extractSearchMetadata } from "../../service/extract-searchMetadata.service";
...
export async function searchSchemes(query: string) {
  ...
  try {
    ...
    const { keywords, filters } = await extractSearchMetadata(normalizedQuery);

    // 3. Fetch schemes (broad match)
    const schemes = await schemeRepository.searchByKeywords(keywords);
    ...
    throw new AppError("Failed to search schemes", 500);
  }
}
 ...
 */
function applyFilters(schemes: Scheme[], filters: any): Scheme[] {
  return schemes.filter(scheme => {
    ...
    if (filters.beneficiaryType) {
        ...
        const hasMatch = 
            scheme.getTags().some(t => t.toLowerCase().includes(bType)) ||
 ...
 */
function rankSchemes(schemes: Scheme[], keywords: string[]): Scheme[] {
  if (keywords.length === 0) return schemes;
...
async function fallbackSearch(query: string) {
  const { schemes } = await schemeRepository.searchSchemes(query, 0, 5);
