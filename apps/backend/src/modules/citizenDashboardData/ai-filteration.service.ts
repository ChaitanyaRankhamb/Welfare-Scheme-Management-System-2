...
import { aiSchemesRecommendationSystemPrompt } from "./openRouter.system.prompt";
...
interface AIResponse {
  results: AIScoreResult[];
}

/* ---------------- MAIN SERVICE ---------------- */

export const aiFilterationService = async (
  userId: UserId,
  ...
  if (!profileData) {
    throw new AppError("Profile data not found", 404);
  }
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: aiSchemesRecommendationSystemPrompt,
          },
          {
            role: "user",
...
function addFallbackScores(schemes: Scheme[]) {
...
function mapSchemeToJSON(scheme: Scheme) {
  return {
    id: scheme.id.toString(),
    title: scheme.getTitle(),
